...
def horstify(contract, dir, results, contract_path):
    ...
    print(f"./horstify {EXEC_FLAG} {dir} {contract_path} {results}")
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hc:t:r:i:a:",
                                   ["contracts=", "timeout=", "results=", "ignore=", "allow="])
    except getopt.GetoptError:
        print(
            f'execute-horstify.py -c <contract-dir> -t <timeout> -r <result-dir> -i <ignore-list> -a <allow-list>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print(
    ...
    # TODO Decide for single result solution (migrate to exec-horst.py) and create .json out files before use
    ...
    shutil.copy2(f"{RULES}", f"{HORSTIFY_DIR}rules")

    # Iterates through all files in the contract directory
    contractlist = Path(contract_dir).glob('**/*.bin.hex')
    for contract in contractlist:
        # because path is object not string
        contract_in_str = str(contract)
        # extract file name from path
        file = str(os.path.basename(contract))
